...
def load_data(train_path, val_path, test_path):
    try:
        ...
        print('No customized dataset provided, using demo data.')
        raw_train = pd.read_csv('gate-model-finetuning-version1/data/demo_train.csv')
        raw_val = raw_train.sample(frac=0.2)
        raw_test = pd.read_csv('gate-model-finetuning-version1/data/demo_test.csv')
...
def prepare_dataset(df, num_samples=None):
    ...
    def clean_task_content(content):
        # Extract the content after "hi" and before "Ground-truth Answer"
        match = re.search(r"hi.*(?=Ground-truth Answer:)", content, re.DOTALL)
        return match.group(0).strip() if match else content

    # Apply the cleaning function to the Task Content column
    df['Reasoner Task Content'] = df['Reasoner Task Content'].apply(clean_task_content)
